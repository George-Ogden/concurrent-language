...
unsigned int ThreadManager::hardware_concurrency()
{
    return std::thread::hardware_concurrency();
}

unsigned int ThreadManager::available_concurrency()
{
    return num_cpus_override.value_or(hardware_concurrency());
}

void ThreadManager::override_concurrency(unsigned int num_cpus)
{
...
unsigned ThreadManager::set_affinity(unsigned cpu_id)
{
    ...
    if (result != 0)
    {
        throw std::runtime_error("Failed to set CPU affinity: " + std::string(strerror(errno)));
    }
    return cpu_id;
}
...
void ThreadManager::thread_setup(size_t cpu_id, bool verbose)
{
    ...
    if (verbose)
    {
        std::cout << "Running on CPU " << cpu << " with priority " << priority << std::endl;
    }
...
auto ThreadManager::thread_run(size_t cpu_id, F &&f, T &&arg,
                               bool verbose)
{
