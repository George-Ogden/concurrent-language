...
impl Refresher {
    ...
    fn refresh_lambda(&mut self, lambda: &mut IntermediateLambda) {
        ...
        self.refresh_statements(&mut lambda.statements, false);
    ...
    pub fn refresh_statements(
        ...
        let extra_statement = if keep_last { statements.pop() } else { None };
        for statement in statements.iter_mut() {
            self.refresh_statement(statement, false);
        }
        if let Some(mut statement) = extra_statement {
            self.refresh_statement(&mut statement, true);
            statements.push(statement);
        }
    }
    fn refresh_statement(&mut self, statement: &mut IntermediateStatement, keep_last: bool) {
        match statement {
            ...
            IntermediateStatement::IntermediateIfStatement(IntermediateIfStatement {
                ...
                self.refresh_statements(&mut branches.0, keep_last);
                self.refresh_statements(&mut branches.1, keep_last);
            }
            IntermediateStatement::IntermediateMatchStatement(IntermediateMatchStatement {
                ...
                for branch in branches {
                    ...
                    self.refresh_statements(&mut branch.statements, keep_last);
                }
            }
        }
    }
